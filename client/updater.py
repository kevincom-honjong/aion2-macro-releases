...
def main():
    ...
    # ══════════════════════════════════════════════════════════════
    # ★★사고 379-b — ★부팅 체크가 살아 있는 매크로의 exe 를 덮으려 했다★★
    #   ★실측 PC-12 (2026-09-01 09:10, 주인님이 로그를 주심)★
    #     09:10:06 매크로 exe: 1.1.823 → 1.1.826
    #     09:10:33 시드 실패(WinError 32) → GitHub 폴백
    #     09:10:42~09:11:30 ★4회 전부 WinError 32★ (다른 프로세스가 파일 사용 중)
    #     09:11:30 ★실패★ — 옛 버전으로 계속한다
    #     09:11:30 [매크로] ★이미 실행 중 (외부 기동 감지) → 중복 기동 생략★
    #   ★매크로가 살아서 exe 를 잡고 있었다.★ 전 함대 24대 중 ★17대★ 가 이렇게 죽었다.
    #
    #   ★부팅 경로는 매크로를 죽이면 안 된다★ — 업데이터가 자기 자신을 올리고
    #   재시작한 것뿐인데 사냥 중인 매크로를 끊으면 그게 더 나쁘다(§A7 정신).
    #   `update` 명령 경로는 `stop_macro()` 로 taskkill 하고 오므로 그때 하면 된다.
    #   → ★매크로가 살아 있으면 exe 교체를 아예 시도하지 않는다.★
    #     4회 재시도(약 90초)를 헛되이 태우고 「실패」로 끝나는 것보다,
    #     ★안 했다고 정직하게 남기고★ 다음 update 명령을 기다리는 편이 낫다.
    # ══════════════════════════════════════════════════════════════
    _skip_boot_upd = False
    try:
        if _macro_running_anywhere():
            _skip_boot_upd = True
    except Exception as _me379:
        err(f"[업데이터] 매크로 생존 확인 실패(그냥 체크한다): {_me379}")
    if _skip_boot_upd:
        log("[업데이터] ★매크로가 이미 돌고 있다 — 시작 업데이트 체크를 건너뛴다★ "
            "(살아 있는 매크로가 exe 를 잡고 있어 덮어쓰기가 WinError 32 로 "
            "실패한다. 대시보드 [업데이트] 명령이 오면 그때 매크로를 끄고 받는다, "
            "사고 379-b)")
        try:
            _set_state("running")
        except Exception:
