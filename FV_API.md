...
| 명령표를 읽지 못함(서버 화면 코드 파싱 실패 — 닫힌 채로 실패, 2026-09-26) | 503 | `{"error": "명령표를 읽지 못했습니다 — 서버 로그 «[FV] 명령표 파싱 실패» 를 보십시오", "code": 503}` — `POST /api/fv/command` 만. 예전엔 전부 통과시켰다 |
...
- ★`why.hand_at` (2026-09-24 팜뷰 #201 r3e p6, 선택·더하기만)★ — 그 판매의 ★게임 안 인계 시각★, epoch 초(숫자, 소수 가능). 모르면 빼고 보낸다(`null` 도 «모름»).
  ...
  ★`hand_at` 은 인계 시각의 ★하한★ 이다(2026-09-25 합동 반증)★ — 팜뷰는 «거래 끝(방 dt)·첫 종료·[인계] 클릭·옮긴 줄의 고른 시각(end_unknown)» 중 ★가장 이른 것에서 10분을 뺀★ 값을 보낸다. 게임 안 인계는 그 어느 것보다 앞이므로 `hand_at` 은 실제 인계보다 ★뒤로 오지 않는다★. 앞으로 올 수는 있는데, 그러면 «판독 전 인계» 로 읽혀 ★덜 뺄 뿐★ 두 번 빼지 않는다. 서버는 이 값을 늦추거나 기록 시각으로 자르지 않는다. 계속 epoch 초(소수 가능)이고, 유효하지 않으면 빼고 보낸다.
...
- ★`auto` 두 번째 길 — 믿을 만한 자리 (2026-09-25 주인님 #228 «OCR 80개 찼다», 더하기만)★ — 맵 이름이 아닌 site 도 사람 라벨 묶음이 200 개 이상이고 ★최근 200 장★(사람이 라벨·나쁨 준 묶음 이미지 중 제미나이 답이 있는 것, ★MAX(라벨 시각, 이미지 도착) 최신순★ — `stats` 불일치율과 같은 비교, 나쁨 묶음은 불일치)이 전부 일치하면, ★새 묶음★ 을 `auto`(label = 제미나이 답)로 닫는다. ★20 번째마다 1건은 `pending`(표본 검사)★. 표본에 사람이 다른 답을 주면 그 site 는 최근 200 장이 다시 깨끗해질 때까지 저절로 꺼진다(새 묶음이 전부 대기열로). `auto` 는 사람 라벨이 아니라 창·불일치율에 안 들어간다.
