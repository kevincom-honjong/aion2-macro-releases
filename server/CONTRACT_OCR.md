...
- ★옛·새 프롬프트 열쇠(매크로 1.1.1006 예시값 교체)★ — `ocr_label.PROMPT_SHA1_OLD_NEW`(21쌍 — 2026-09-24 awakening.py:294 반증 2차 더함, 정본 표 = `src/SHARED_ISSUES_아이온2.md` «제미나이 프롬프트 예시값 교체», 시험 A-1 이 두 표를 대조)에 있는 `prompt_sha1` 이면 같은 행을 **다른 판 열쇠로도 하나 더** 보낸다(`alias_of` = 원래 열쇠, 같은 sha1·라벨·ts·deleted). 새 키 `62fd6da2e23d`(키나 세 칸 합침)는 옛 키 셋 모두로. 함대가 옛·새 판 섞여 돌아도 어느 쪽 라벨이든 적중.
