...
<div id="card-menu" class="hidden fixed z-50 cm-panel" onclick="event.stopPropagation()">
  ...
  <!-- ★PIN 막힘 풀기 (2026-09-26 PIN 3차 · SHARED_ISSUES_아이온2 «pin_reset»)★ — 맞는 PIN 인데 막힌 계정의 PIN 예산
       (확인된 실패·사람 대기·재요구 기록)을 매크로가 푼다. 매크로가 인라인 처리(도는 작업을 안 끊는다). 비밀 없음.
       ★한 PC 명령 — 일괄·함대 버전은 만들지 않는다★. 버튼은 JS refreshPinButtons() 가 MAX_ACCT 에서 만든다. -->
  <div class="cm-sec">PIN 막힘 풀기 (그 계정만)</div>
  <div class="cm-grid2">
    <span id="cm-pin-box" style="display:contents"></span>
  </div>
...
async def _dispatch_macro_command(tenant: str, pc_id: str,
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ★★사고 523 — ★사람이 보낸 것★ 이라고 매크로에 말해준다 (2026-09-09)★★
    #   ★주인님★ 「사냥시작을 시켯는데 왜 자꾸 계정전환하려고… ★사냥시작
    #              오버라이드가 안되는거야?★」  (그 전 지시: 2026-09-08
    #              「내가 명령을 시키면 그냥 idle 상태로 만들거나 ★오버라이드 해★」)
    #
    #   ★왜 표식이 필요한가★ 매크로는 「이전 명령 처리 중」이면 새 명령을 거부한다
    #   (loot.py 사고 489). 그 규칙은 ★순환 엔진이 스스로 보내는 명령★ 에는 옳다 —
    #   그때 끼어들면 두 워커가 같은 화면을 만진다(§A11). 그런데 ★사람이 누른 것★
    #   까지 같이 거부해서, 주인님이 ▶시작을 눌러도 아무 일이 안 났다
    #   (PC-16 14:36:02·14:36:03 실측 — 두 번 다 거부).
    #
    #   ★여기가 사람 경로다★ — `_rot_send`(순환 엔진)는 이 함수를 안 거치고
    #   `send_command_to_macro` 를 직접 부른다. 그래서 여기 오는 것은
    #   대시보드 · FV API · ops 도구 = ★전부 사람이 시킨 것★ 이다.
    #
...
ROT_SESSION      = ("corridor", "nightmare", "dungeon", "awakening", "surface")   # 세션 중 — 사냥 아님, 끝나길 기다린다
#   ★surface (#208, 2026-09-25)★ 표층 세션 — 매크로가 회랑처럼 보고한다. 빠지면 순환이 «사냥 안 잡힘»/딴 일로 읽고
#   7분 뒤 ⛔ 하거나 세션 위에 ▶start 를 보낸다. ROT_HUNT_LIKE·ROT_SESS_HOLD 는 여기서 따라온다.
...
#
#   ★무엇이 있었나 (PC-16 14:15:47 실측)★ 파섹 네이티브로 막 붙어서
#   「✔ 04 파섹 접속」 을 찍은 ★바로 그 초★ 에 18시간 TTL 이 걸려 순환이 해제됐다.
#   전환은 정상으로 굴러가는 중이었다 — ★고장이 아니라 시계 때문에 꺼진 것이다.★
#
...
#
#   ★남는 위험 (숨기지 않는다)★ 주인님이 ★매크로 핫키★ 로 멈춰두고 손보시는 동안에는
#   해제 신호가 서버로 안 온다(그 경로는 /command 를 안 거친다). 예전엔 TTL 이 18시간
#   뒤 그 무장을 걷어갔는데 이제 안 걷는다 — ★손볼 땐 대시보드에서 ⏹정지를 눌러야 한다.★
# ══════════════════════════════════════════════════════════════════════════════
...
ROT_ST_KOR = {"hunting": "사냥", "moving": "이동", "selling": "판매",
              ...
              "awakening": "각성전", "corridor": "회랑", "surface": "표층 진입중", "abyss": "어비스",
...
def _rot_active(cards: list) -> dict | None:
    ...
    ★두 가지를 같이 본다 [S3 / 리뷰 ②]★
      ① status 가 other_account/offline/no_account 이 아닐 것
    ...
    for c in cards:
        st = str(c.get("status"))
        if st in ("other_account", "offline", "no_account"):
            continue
...
async def _rot_step_pc(tenant: str, base: str, st: dict, pcs: list) -> None:
    ...
    # ★[M1] 여기 있던 TTL 자동 해제는 없앴다 (2026-09-09 주인님 지시)★ — 상수 자리 묘비 참고.
    #   ★[M1] 이 잡던 「카드가 사라진 좀비 엔트리」는 그대로 남는다★ — 아래 `if not cards`
    #   에서 아무 명령도 안 쏘고 물러나므로 ★해를 끼치지 않는다.★ 무장만 살아 있을 뿐이다.
    ...
    if stage == "collecting":
        ...
        #   아니라 ★18시간 TTL 까지 무음★ 이었다(그 TTL 은 2026-09-09 에 없앴다 —
        #   이제 이 타임아웃이 없으면 ★영영 무음★ 이다). 다른 단계는 타임아웃이 위에 있다.
    ...
    if stage == "switching":
        ...
        if active and _rot_acct_no(active.get("pc_id")) == want_no:
            ...
            #   증상: 20분 뒤 떠야 할 "⛔ 계정N 전환이 20분째" 알람이 안 뜬다.
            #   (예전엔 그 뒤 18시간 TTL 이 ★원인과 무관한 문구★ 로 끝냈다. TTL 은
...
def _fv_parse_cmd_table() -> dict:
    ...
    try:
        ...
        if blk:
            # ★줄 끝 `// 주석` 을 허용한다 (#208, 2026-09-25)★ — 예전 식은 `},` 뒤 공백만 받아 주석 달린 줄(surface, 사고 574)이
            #   명령표에서 ★통째로 빠졌다★(팜뷰 명령 목록에 표층 없음). 이제 CMD_TRACK 키 수 == 명령표 키 수를 시험이 본다.
    ...
    for name in _FV_EXCLUDED_CMDS:
        out.pop(name, None)
    return out


_FV_CMDS: dict = {}
# ★팜뷰 명령표에서 빼는 명령★ — 명령표는 CMD_TRACK/CMD_SILENT 를 파싱해 만들므로 화면에 넣는 순간 팜뷰에도 열린다.
#   pin_reset(2026-09-26 PIN 3차)은 ★한 PC 명령★ 이라(아이온2 지시 «no fleet or bulk variant») 팜뷰 다중 PC 로는 안 연다
#   (반증 1 — /api/fv/command 는 7대까지 확인 없이, confirm_fleet 이면 함대 전체). 대시보드 카드 메뉴에서만.
_FV_EXCLUDED_CMDS = frozenset({"pin_reset"})
