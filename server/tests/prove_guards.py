...
CASES = [
    ...
    ("kina_adjust tid 중복 가드 제거", "database.py",
     "        if seen:\n            await db.execute(\"ROLLBACK\")\n            return {\"pc_id\": pc_id, \"before\": before, \"after\": before, \"dup\": True}\n",
     "", "test_contracts.py"),
    ("kina_adjust 바닥 0 제거", "database.py",
     "        after = max(0, before + int(delta))", "        after = before + int(delta)", "test_contracts.py"),
    ("kina_age_s 모름=10^9 제거", "main.py",
     "    if a is None:\n        return FV_AGE_UNKNOWN_S\n    return max(0, int(a))", "    return max(0, int(a or 0))", "test_contracts.py"),
